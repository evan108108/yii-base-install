...
class m111222_183233_create_user_tables extends CDbMigration
{
	public function up()
	{
		...
		$sql = "ALTER TABLE user ADD CONSTRAINT uc_username UNIQUE (username)";
		$command = $this->getDbConnection()->createCommand($sql);
		$command->execute();	

		$sql = "ALTER TABLE user ADD CONSTRAINT uc_email UNIQUE (email)";
		$command = $this->getDbConnection()->createCommand($sql);
		$command->execute();	

		$sql = "INSERT INTO user (`id`,`username`,`email`,`password`,`activkey`,`createtime`,`lastvisit`,`superuser`,`status`) VALUES (1,'admin','webmaster@example.com','21232f297a57a5a743894a0e4a801fc3','9a24eff8c15a6a141ece27eb6947da0f',1261146094,0,1,1)";
		$command = $this->getDbConnection()->createCommand($sql);
		$command->execute();

		$sql = "INSERT INTO user (`id`,`username`,`email`,`password`,`activkey`,`createtime`,`lastvisit`,`superuser`,`status`) VALUES (2,'demo','demo@example.com','fe01ce2a7fbac8fafaed7c982a04e229','099f825543f7850cc038b90aaff39fac',1261146096,0,0,1)";
		$command = $this->getDbConnection()->createCommand($sql);
		$command->execute();
